...
//   // load background
//   JpgScaleData textures = JpgImageScale::load(Tyra::FileUtils::fromCwd(background1unsodded).c_str(),TextureScale::Tex128,TextureScale::Tex128);
//   SetBigImage(&Entity::backgroundIDs, &textures,SpriteMode::MODE_STRETCH,-141,0,81,95);
...
//   // createSprite(awardBackground, MODE_REPEAT, Vec2(255, 410), Vec2(158, 24));
//   // createTexture(awardBackground, "Images/AwardScreen_Back.jpg");
...
//   zombieCreateRow[2].maxZombiesInRow = 10;
//   zombiescreated = 0;
//   newCursor(&Entity::player, Vec2(mapCollider[0][0].x, mapCollider[0][0].y + 30));
//   newDeckCursor(&Entity::player,
//                 Vec2(posArray[cards[deckCursor[Entity::player].pos].seed].x - 3, -10));
...
//   printf("sodnormal texture: %d\n",textures.data.size());
//   SetBigImage(&sodnormal, &textures,SpriteMode::MODE_REPEAT,13,200,256,128);
//   for(unsigned int i=0;i<sodnormal.id.size();i++){
//     // TODO: el size x  e  y deben ser 0
//     spriteArray[sodnormal.id[i]].size.x = -1.0f; 
//     spriteArray[sodnormal.id[i]].scale = 0.5f;
//   }
//   deleteSprite(Entity::sodRollRow1);
//   deleteSprite(Entity::sodRollRow1Alpha);
//   Entities::deleteID(Entity::sodRollRow1);
...
//   static int frame=0;
//   frame = 0;
//   // TODO: se muestra mal porque la posicion no fue modificada por el size
//   if(spriteArray[sodnormal.id[0]].size.x<256){
//     posArray[sodRoll].x+=5;
//     posArray[sodRollCap].x+=5;
//     spriteArray[sodnormal.id[0]].size.x+=5; 
//     // spriteArray[sodRollRow1].size.x+=5;
//   }else if(spriteArray[sodnormal.id[1]].size.x<256){
//     posArray[sodRoll].x+=5;
//     posArray[sodRollCap].x+=5;
//     spriteArray[sodnormal.id[1]].size.x+=5;      
